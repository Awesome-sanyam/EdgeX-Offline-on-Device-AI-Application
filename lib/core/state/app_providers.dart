...
class SessionsNotifier extends Notifier<List<ChatSessionMeta>> {
  ...
  List<ChatSessionMeta> build() {
    ...
    if (saved != null) {
      return saved
          .map((e) => ChatSessionMeta.fromJson(jsonDecode(e)))
          .toList();
    }
    return [];
  }
  ...
  void createOrUpdateSession(String id, String title) {
    ...
    if (existingIndex != -1) {
      updatedList[existingIndex] = ChatSessionMeta(
        id: id,
        title: title,
...
class ChatNotifier extends Notifier<List<ChatMessage>> {
  ...
  Future<void> sendMessage(String text, String? attachmentPath) async {
    ...
    if (attachmentPath != null &&
      ...
      try {
        ...
        if (extractedText.isEmpty) {
          ...
          finalPrompt =
              'Please use the following document text to answer my prompt.\n\n<document>\n$extractedText\n</document>\n\nMy Prompt: $text';
        }
      } catch (_) {
        finalPrompt +=
    ...
    // Small delay to let the UI frame render before heavy computation
    ...
    try {
      ...
      state = [
        ...state,
        ChatMessage(id: aiMsgId, text: '', isUser: false),
      ];
      ...
      fllamaChat(request, (String response, String role, bool isDone) {
        ...
        // OPTIMIZATION: Only rebuild UI if 40ms elapsed OR generation is done.
        // This prevents flooding the widget tree with near-empty token updates.
